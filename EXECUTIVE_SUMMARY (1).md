...
- **Monte Carlo does not model contribution schedule changes.** The simulation holds the savings rate constant across the full projection horizon. A more accurate model would step the rate up at promotions and down at separation, mirroring the actual contribution behavior the solver computes.
